...
def filter_components(num_labels, labels, stats, centroids, min_area_threshold=15, rows=8, cols=8):
    filtered_labels = np.zeros_like(labels, dtype=np.int32)

    # Initialize lists to store filtered stats and centroids (including background at index 0)
    ...
    for i in range(1, num_labels):
        ...
        if area >= min_area_threshold:
            filtered_labels[labels == i] = current_label
